...
*Setup instructions for a Gitian build of worldcoin Core using a Debian VM or physical system.*
...
- [Building worldcoin Core](#building-worldcoin-core)
...
Clone the git repositories for worldcoin and Gitian.
...
in the worldcoin repository under 'Fetch and create inputs' to install sources which require
...
To build worldcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the worldcoin repository.
...
For example:
```bash
URL=https://github.com/thrasher-/worldcoin.git
...
and the worldcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../worldcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/worldcoin
SIGPATH=/some/root/path/worldcoin-detached-sigs

./bin/gbuild --url worldcoin=${BTCPATH},signature=${SIGPATH} ../worldcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/worldcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/worldcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/worldcoin-osx-build.assert
```
...
[worldcoin-project/gitian.sigs.ltc](https://github.com/worldcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
