...
for Ubuntu Xenial does not produce working executables for some of the worldcoin Core applications.
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/worldcoin-project/worldcoin.git
    sudo chmod -R a+rw worldcoin
...
causes two of the worldcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the worldcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the worldcoin source code).
